...
function App() {
  ...
  const setupStreamAutoZoom = (source) => {
    // Already processed this source
    if (autoZoomProcessedSources.current.has(source.source_id)) {
      return;
    }
    
    console.log(`Setting up auto-zoom for new stream: ${source.name}`);
    autoZoomProcessedSources.current.add(source.source_id);
    
    const startTime = Date.now();
    let checkCount = 0;
    
    // Check condition: 2 seconds elapsed OR 50 targets processed
    const checkInterval = setInterval(async () => {
      checkCount++;
      const elapsed = Date.now() - startTime;
      
      // Get current target count for this source
      const currentSource = sources.find(s => s.source_id === source.source_id);
      const targetCount = currentSource?.target_count || 0;
      
      if (elapsed >= 2000 || targetCount >= 50) {
        clearInterval(checkInterval);
        console.log(`Auto-zoom triggered for ${source.name}: ${elapsed}ms elapsed, ${targetCount} targets`);
        
        // Small delay to ensure vessels are loaded
        setTimeout(() => autoZoomToSource(source.source_id), 200);
      } else if (checkCount > 10) {
        // Safety: stop checking after 10 iterations (10 seconds)
        clearInterval(checkInterval);
        console.log(`Auto-zoom timeout for ${source.name}`);
      }
    }, 1000);
  };
