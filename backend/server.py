...
# Load Marinesia environment variables
load_dotenv(ROOT_DIR / '.env.marinesia')

# Import Marinesia client
try:
    from marinesia_client import MarinesiaClient
    ...
    if MARINESIA_ENABLED and MARINESIA_API_KEY:
        marinesia_client = MarinesiaClient(MARINESIA_API_KEY)
        print("✅ Marinesia integration enabled")
    else:
        marinesia_client = None
        print("Marinesia integration disabled")
except Exception as e:
    marinesia_client = None
    print(f"Marinesia client not available: {e}")
...
async def process_ais_message(raw_message: str, source: str = "unknown", source_id: str = None, log_timestamp=None):
    ...
    try:
        ...
        # Use timestamp priority: log_timestamp > message UTC sync > system time
        if log_timestamp:
...
async def get_active_vessels(
    ...
    try:
        ...
        for source in active_sources:
            ...
            # Only calculate ranges for FILE sources, not streams
            is_file_source = source.get('source_type') == 'file'
            should_calculate_ranges = is_file_source
            
            if not is_file_source:
                logger.info(f"Source {source_id} is a stream - skipping range calculations")
            
            # For file sources, also check if multiple base stations at different locations
            has_multiple_base_stations = False
            if is_file_source:
                own_vdo_locations = {}
                ...
                if has_multiple_base_stations:
                    should_calculate_ranges = False
                    logger.info(f"File source {source_id} has multiple VDO base stations - skipping range calculations")
            ...
            for vdo_pos in vdo_positions:
                ...
                # Only calculate for file sources (not streams)
                max_distance_within_limit = 0
                
                if is_own_base_station and should_calculate_ranges:
...
async def get_vessel_track(mmsi: str, limit: int = 10000):
    ...
    try:
        ...
        positions = await db.positions.find(
            {'mmsi': mmsi},
            {'lat': 1, 'lon': 1, 'timestamp': 1, 'speed': 1, 'course': 1, 'heading': 1, 'source_id': 1, 'source': 1}
